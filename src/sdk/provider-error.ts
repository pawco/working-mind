...
function extractOllamaMessage(status: number, body: string): string {
	const parsed = tryParseJson(body);
...
export function classifyProviderError(err: any): ClassifiedError {
	...
	if (status === 429 || errName === 'RateLimitError' || errCode === 'rate_limit' || errType === 'rate_limit_error') {
		return {
			category: 'rate_limit',
			statusCode: 429,
			providerMessage: msg,
	...
	if (status === 401 || errName === 'AuthenticationError' || errCode === 'invalid_api_key' || errType === 'authentication_error') {
		return {
			category: 'auth',
			statusCode: 401,
			providerMessage: msg,
	...
	if (status === 404 || errName === 'NotFoundError' || errType === 'not_found_error') {
		const isOllama = errName === 'OllamaProviderError' || msgLower.includes('model') || msgLower.includes('ollama');
		return {
			category: 'not_found',
			statusCode: 404,
			providerMessage: msg,
	...
	if (msg.startsWith('Ollama error')) {
		...
		if (match) {
			...
			return {
				...
				statusCode: s,
				providerMessage: oMsg,
				suggestion: s >= 500 ? 'Ollama server error. Check if Ollama is running.' : 'Check Ollama logs for details.',
				canRetry: s >= 500,
			};
		}
	}
...
export function formatErrorForDisplay(classified: ClassifiedError): string {
	...
	if (classified.statusCode) {
		const idx = parts[0].includes(`HTTP ${classified.statusCode}`)
